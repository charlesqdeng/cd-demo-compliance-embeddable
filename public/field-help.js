...
function showGuidanceModal(guidance) {
    ...
    if (guidance.validationFailures) {
        html += `<div class="guidance-section guidance-mistakes">
            <h4>${guidance.validationFailures.title}</h4>
            <ul>
                ${guidance.validationFailures.reasons.map(reason => `<li>${reason}</li>`).join('')}
            </ul>
        </div>`;
    }

    if (guidance.manualReview) {
        html += `<div class="guidance-section" style="background: #FFF4E5; border-left: 4px solid #FF9800; padding: 1rem; border-radius: 4px;">
            <h4>${guidance.manualReview.title}</h4>
            <p><strong>${guidance.manualReview.note}</strong></p>
            <ul>
                <li><strong>U.S. entities:</strong> ${guidance.manualReview.usEntities}</li>
                <li><strong>Non-U.S. entities:</strong> ${guidance.manualReview.nonUsEntities}</li>
            </ul>
            <p><strong>How to submit:</strong> ${guidance.manualReview.instructions}</p>
            <p style="margin-top: 0.5rem;"><em>💡 ${guidance.manualReview.tip}</em></p>
        </div>`;
    }
