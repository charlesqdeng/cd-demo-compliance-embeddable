...
async function handleFormSubmit(e) {
    ...
    try {
        ...
        if (result.success) {
            ...
            // Pass the full result object to show enhanced error messages
            throw { response: result, message: result.message };
        }
    } catch (error) {
...
function showErrorMessage(error) {
    ...
    // Check if this is a "verification already exists" error
    const isVerificationExists = error.error === 'VERIFICATION_EXISTS';
    const icon = isVerificationExists ? 'ℹ️' : '⚠';
    const iconColor = isVerificationExists ? '#FF9800' : '#D61F1F';
    const title = isVerificationExists ? 'Verification Already Exists' : 'Submission Error';

    let detailsHTML = '';
    if (error.details) {
        detailsHTML = `<p style="color: #606B85; margin-bottom: 1rem; font-size: 0.95rem;">${error.details}</p>`;
    }

    let actionsHTML = '';
    if (error.actions && error.actions.length > 0) {
        actionsHTML = `
            <div style="background: #F4F4F6; padding: 1.5rem; border-radius: 8px; margin: 1.5rem 0; text-align: left;">
                <h4 style="color: #121C2D; margin: 0 0 1rem 0; font-size: 1rem;">What you can do:</h4>
                <ul style="margin: 0; padding-left: 1.5rem; color: #606B85;">
                    ${error.actions.map(action => `<li style="margin: 0.5rem 0;">${action}</li>`).join('')}
                </ul>
            </div>
        `;
    }

    let consoleButtonHTML = '';
    if (error.twilioConsoleUrl) {
        consoleButtonHTML = `
            <a href="${error.twilioConsoleUrl}" target="_blank" style="
                ...
                margin-right: 1rem;
                transition: background 0.3s;
            ">Open Twilio Console</a>
        `;
    }

    container.innerHTML = `
        <div style="text-align: center; padding: 3rem;">
            <div style="font-size: 4rem; color: ${iconColor}; margin-bottom: 1rem;">${icon}</div>
            <h3 style="color: #121C2D; margin-bottom: 1rem;">${title}</h3>
            <p style="color: #606B85; margin-bottom: 1rem;">
